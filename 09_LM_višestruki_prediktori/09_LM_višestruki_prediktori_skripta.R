...
levels(dat$gender)
#Učitajte ponovo model! Što se promjenilo zamjenom referentne kategorije spola?

#d3) izrada modela ovisnosti gubitka mase o vremenu vježbanja i spolu
contcat <- lm()
anova()
summary()

#d7) vizualizacija koristeći naredbu  emmip
...
#e2) izrada modela: ovisnost gubitka mase o programu
catcat <- lm()
anova()
summary()
...
# 1. Napravite model gubitka mase u ovisnosti o svim faktorima u tablici, ali bez interakcija. Koje su varijable značajne za gubitak mase? Je li model značajan? Koliko udio varijacije je objašnjen modelom?
# 2. Napravite model gubitka mase u ovisnosti o svim faktorima u tablici i njihovim interakcijama! Koje su varijable značajne za gubitak mase? Je li model značajan? Koliko udio varijacije je objašnjen modelom?
# 3. Napravite model gubitka mase u ovisnosti o svim faktorima u tablici, ali interakcijom faktora "effort" i "hours" te "prog" i "gender"! Koje su varijable značajne za gubitak mase? Je li model značajan? Koliko udio varijacije je objašnjen modelom?
